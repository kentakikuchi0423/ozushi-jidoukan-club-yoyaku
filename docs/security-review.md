...
| 管理者による予約キャンセル | 済 | `admin_cancel_reservation(uuid)` RPC は `revoke all from public` + `grant execute to service_role` で、admin client（secret key）からのみ呼べる。Server Action `adminCancelReservationAction` が render / submit の両方で `requireFacilityPermission(facilityCode)` を実施し、別館 admin が任意の reservation_id を URL 直叩きしてもキャンセルできない。確認画面で権限なし時は `notFound()` 扱いで予約存在の有無を伏せる（ADR-0021） |
...
| Brute Force（管理者ログイン） | 済 | Supabase Auth の既定レート制限 + `admin.login.succeeded` / `admin.login.failed` を `audit_logs` に全件記録（email / IP / reason）+ 同一メールが直近 30 分で 5 回失敗した時点で本人へ注意喚起メール（ADR-0033、`evaluate_login_alert` RPC + `login-alert.ts`、未登録メールには送らず 24h cool-down）。異常検知は `docs/operations.md §10` の tail 手順で月次運用、将来必要なら Edge Middleware で IP ベース制限を追加 |
...
| 2026-04-26 | Claude | 管理者キャンセル導線 | `admin_cancel_reservation(uuid)` を service_role のみ EXECUTE 可とし、Server Action 側で `requireFacilityPermission` を render / submit の両方で実施。権限外の reservation_id 直叩きには `notFound()` で予約有無も伏せる。監査ログ `reservation.admin_cancel` を追加（個人情報を metadata に含めない、ADR-0021） |
| 2026-04-26 | Claude | 待ちリスト再採番ヘルパー追加 | `renumber_waitlist_after_gap(uuid, integer)` を新設し `cancel_reservation` / `admin_cancel_reservation` から呼び出す。`revoke all from public` で外部公開せず（service_role からも直接呼ぶ用途なし）。挙動上の機微情報露出は無いが、`(club_id, waitlist_position)` 一意制約を破らない順序で更新する点を ADR-0022 に明記 |
| 2026-04-28 | Claude | ログイン失敗アラート | 同一メール × 直近 30 分で 5 回失敗時に本人へ注意喚起メール（ADR-0033）。`evaluate_login_alert` RPC（service_role のみ実行可）で 1 回問い合わせて判定、未登録メールには送らず 24h cool-down で連投を抑止、本文には IP / 時刻 / 失敗回数を含めない。Brute Force ステータスを「済（最小限）」→「済」に格上げ |
