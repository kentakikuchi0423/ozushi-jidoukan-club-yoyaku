...
| メール | Resend 6 | 6 テンプレート（confirmed / waitlisted / promoted / canceled / admin-invite / login-alert）。利用者向け 4 本は text + HTML を multipart で送信（ADR-0016）。login-alert は管理者ログイン失敗 5 回／30 分で発火（ADR-0033） |
...
├── supabase/
│   ├── config.toml
│   ├── migrations/                        # 18 本（initial / rpcs / retention / listing / detail / lookup / fixes / multi_parent_child / fk_cleanup / optional_parents / club_programs / get_my_reservation_program / clubs_published_at / facility_phone_and_dynamic / admin_cancel_reservation / renumber_waitlist_on_cancel / list_public_clubs_stable_sort / login_alert_helpers）
...
| `evaluate_login_alert(email, window_minutes, cooldown_hours)` | admin client（service_role） | ログイン失敗アラート判定。admins 存在 + 失敗件数 + 直近通知の有無を 1 回で返す（ADR-0033） |
...
- アカウント: `admin.create` / `admin.password_change` / `admin.login.succeeded` / `admin.login.failed` / `admin.login.alert_sent`
...
| `admin.login.alert` | `login-alert.ts` | 当該 admin | ログイン失敗 5 回/30 分で送る注意喚起。失敗回数・IP・時刻は本文に出さない（ADR-0033） |
...
管理者向け 2 本（admin-invite / login-alert）は別系統のテキスト構造のため
`textToHtml` 経由で HTML 化する。login-alert は `admins` テーブル存在チェック
（未登録メールには送らない）と 24 時間 cool-down（同一アドレスへ連投しない）を
RPC `evaluate_login_alert` で 1 回にまとめて判定する。
