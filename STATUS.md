...
### このチャンクで解消したもの
0. **協議資料に不要なセクションを削除**:
   - **§8-3 試験中のデータの取り扱い**: 削除方針で協議済みのため本資料からは削除
   - **付録 B 保守事業者側で並行して進める作業**: 保守側内部 TODO で市協議には不要なため削除
1. **§4-4「市の DNS 管理者は誰か」と §7「代替可能性」を行動志向に書き換え**:
   - 各表の「所要日数の目安」「切替工数の目安」列を **「必要な対応」** に置き換え
   - §4-4: パターン A/B/C それぞれで「市側が誰に対して何を依頼するか」を具体化（庁内完結 / 委託業者への作業依頼書 / クラウド契約範囲確認）
   - §7: 各ケースで「市側の対応」「保守側の対応」を 1 セルに併記（IAM 権限付与 / DB 接続情報提供 / SMTP ライセンス確認 など）
   - 第三者がリードタイムではなく「**何をすればよいか**」を読み取れる構造に
1. **`docs/city-consultation.md` をすっきり再構成**（前回分）:
   - 構成を「**前半=§1〜§4 決定事項 / 後半=§5〜§10 参考情報**」の二段に分離
   - §1 サマリから維持費を独立（5 行の決定事項表に圧縮）
   - **§6 維持費**: 4 シナリオ（A/B/C/D）→ 想定シナリオ 1 つ（Vercel Pro のみ ¥38,400/年）+ 各サービスの選択理由 1 行ずつ。ドメインは市既存利用前提で 0 円
   - **§7 代替可能性**: 「市が X を使っているなら → Y で代替できる」の早見表に再構成
     - AWS / Azure / GCP / 自治体クラウド → Vercel 代替
     - 既存 PostgreSQL → Supabase の DB 部分代替
     - M365 / Workspace → Resend SMTP リレー or 認証統合
     - SendGrid / SES / Postmark / Brevo → Resend 代替
   - 代替できないもの（SQL Server / Oracle / MySQL / WordPress / 静的のみ）も明記
   - §10 確認事項: 24 件 → 12 件（必ず確認 / 本格運用前 / 任意 の 3 段）
   - 詳細料金体系は **付録 A** に移動して本論をすっきりさせた
   - **835 行 → 357 行（−57%）**
1. **`docs/city-consultation.md` を非 IT 担当者向けに大幅加筆**（前回分）:
   - 市職員・児童館担当者・意思決定者が読むことを想定し、専門用語に説明を併記
   - 「市のドメインを使う」を「メール差出人ドメイン」と「予約サイト URL」の 2 種類に分けて整理（今回の主論点は前者と明示）
   - §2 として **「サービスを支える 4 サービスとそれぞれの役割」** を新設（郵便のたとえ + 設定先表）。Vercel / Supabase / Resend / Cloudflare Registrar / 市 DNS の役割の混同を防ぐ
   - §4-3〜4-9 で **誰が何をどこに設定するか** をステップバイステップで解説（Resend / 市 DNS / Cloudflare Registrar / Vercel / Supabase / 既存メール基盤の流用可能性）
   - §9 確認事項を 20 件に拡充、§10 として **「当日までのチェックリスト」** を新設（A. DNS / B. 既存メール / C. 個人情報 / D. 本格運用方針）
   - 既存の SPF/DKIM/DMARC 解説、コスト試算、選択肢 A〜D は維持
   - 426 行 → 643 行
1. **`docs/open-questions.md` の完全 close**:
